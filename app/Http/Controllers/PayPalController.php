    ...
    public function paymentSuccess(Request $request)
    {
        ...
        if (isset($response['status']) && $response['status'] == 'COMPLETED') {
            if (session('form_type') === 'donation') {
                Donation::create(session('form'));
                session()->flush();
                return redirect()->route('donation.index')->with('success', 'Transaction complete.');
            } else {

                $member = User::create(session('form'));

                $profilePhotoPath = session('profilePhotoPath');
                $licensePath = session('licensePath');

                if ($profilePhotoPath) {
                    $member->addMedia(storage_path('app/' . $profilePhotoPath))
                        ->withResponsiveImages()
                        ->toMediaCollection('profile');

                    // Delete the temporary file after adding it to the media collection
                    Storage::delete($profilePhotoPath);
                }

                if ($licensePath) {
                    $member->addMedia(storage_path('app/' . $licensePath))
                        ->withResponsiveImages()
                        ->toMediaCollection('license');

                    // Delete the temporary file after adding it to the media collection
                    Storage::delete($licensePath);
                }
